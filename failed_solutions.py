...
def recursive_single_matches(point_match, point, posible_matches, matches, A):
    ...
    if posible_match:
        matches, posible_matches, A = recursive_single_matches(
            posible_match[1].pop(), posible_match[0], posible_matches, matches,
            A
        )
...
def find_max_matching(A, B):
    ...
    for point in B:
        point_matches = set()
        for point2 in A:  # busco los matches posibles para point y
            # los agrego a un set
        ...
        if (
            len(point_matches) == 1
        ):  # si hay un solo match, lo agrego a matches y
            # saco ese punto de posible_matches y de A
            # ademas itero recursivamente para ir agregando
            # todos los puntos que vayan quedando con un solo match
            matches, posible_matches, A = recursive_single_matches(
                point_matches.pop(), point, posible_matches, matches, A
            )
        elif len(point_matches) > 1:
            # si tiene mas de un match,
            # solo se agrega a la lista de posibles matches
...
def find_max_matching2(A, B):
    ...
    while go:
        ...
        for point in posible_matches_A:
            if (
                len(posible_matches_A[point]) < minimum_matches
                and len(posible_matches_A[point]) > 0
            ):
                minimum_matches = len(posible_matches_A[point])
                point_match = point
        for point in posible_matches_B:
            if (
                len(posible_matches_B[point]) < minimum_matches
                and len(posible_matches_B[point]) > 0
            ):
