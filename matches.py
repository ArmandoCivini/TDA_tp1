...
def sort_sets(A, B):
    ...
    A.append(
        (-math.inf, -math.inf)
    )  # truco para que en la ultima iteracion use todos los puntos de B
    return A, B
...
def get_all_possible_B(B, x_start, x_end, curr_B_index):
    possible_B = (
        []
    )  # heap, acumulo todos los puntos de B
    # que estan en el rango de x del A actual y el siguiente
    # se ordenan en y de menor a mayor y se descartan al final de la iteracion

    for point_B in B[
        curr_B_index:
    ]:  # busco todos los puntos de B entre x_start y x_end
...
def find_max_matching(A, B):
    ...
    matches = []
    available_points = SortedList(
        key=lambda point: point[1]
    )  # heap, ordena en de menor a mayor valores de y
    ...
    for i in range(len(A) - 1):
        ...
        possible_B, curr_B_index = \
            get_all_possible_B(B, x_start, x_end, curr_B_index)
